		...
		for sensor_num,sensor in enumerate(self.sensors_object_list):
			while len(sensor)>0:
				...
				for remaining_sensor in self.sensors_object_list[sensor_num:]:
					remaining_sensor_reading_index = 0
					while remaining_sensor_reading_index < len(remaining_sensor):
						remaining_sensor_reading = remaining_sensor[remaining_sensor_reading_index]
					#for remaining_sensor_reading in remaining_sensor:
						if reading_parameters[1] == remaining_sensor_reading.number:
							...
							remaining_sensor.pop(remaining_sensor_reading_index)
						else:
							remaining_sensor_reading_index += 1
